function endsWithHypen(word) {
  return isHypen(lastChar(word))
}
...
function isHypen(letter) {
  return letter == '-'
}

function isLower(letter) {
  return 'abcdefghijklmnopqrstuvwxyz'.split('').includes(letter)
}
...
function twoConsecutiveHypens(name) {
...
function validChar(letter) {
  return isUpper(letter) || isLower(letter) || isHypen(letter) || isSpace(letter)
}
